...
//웹소켓 연결 이후 연결을 처리하는 핸들러를 의미
//TextWebSocketHandler를 상속받아서 최초 소켓 세션을 연결하고 소켓/전송 오류가 발생했을 때 및 '텍스트 기반 메시지'를 보내거나
//받을 수 있도록 처리를 가능하게 한다.
...
//    연결성공 -> 웹소켓 협상이 성공적으로 완료되고 웹소켓 연결이 열려 사용할 준비가 된 후 호출됨
//    성공을 하였을 경우 session값을 추가
...
//    소켓 종료 및 전송오류
//    웹소켓 연결이 어느 쪽에서든 종료되거나 전송 오류가 발생한 후 호출
